export default function decorate(block) {
  ...
  containerSection.innerHTML = `
     ...
     <div class="md:px-14 flex overflow-y-scroll sm:grid md:grid-cols-2 xl:grid-cols-4 gap-8 py-14">
          ${processedTeams.length > 0 ? processedTeams.map((team, index) => `
              ...
              data-index="${index}" 
              id="open-popup" 
              class="flex justify-end text-start flex-col h-[300px] min-w-62 lg:h-[400px]  relative bg-center bg-cover" style="background-image: url('${team.image}')">
  ...
  containerSection.querySelectorAll('#open-popup')
    .forEach(button => button.addEventListener('click', (e) => {
        const index = e.currentTarget.getAttribute('data-index');
        const team = processedTeams[index];
        containerSection.querySelector('#popup')
          .classList
          .toggle('hidden');
  ...
  containerSection.querySelector('#close-button-popup')
  ...
  if (!aemEnv) {
    ...
    block.querySelectorAll(':scope > div:nth-child(n+1)')
