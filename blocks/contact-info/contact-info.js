...
export default async function decorate(block) {
    const mapConfig = {
        ...
        lat: parseFloat(block.querySelector('div:nth-child(2)')?.textContent.trim() || '41.9028'),
        lng: parseFloat(block.querySelector('div:nth-child(3)')?.textContent.trim() || '12.4964'),
        zoom: parseInt(block.querySelector('div:nth-child(4)')?.textContent.trim() || '15', 10),
        mapStyle: block.querySelector('div:nth-child(5)')?.textContent.trim() || 'default',
    };

    const sectionTitle = block.querySelector('div:nth-child(6)')?.textContent.trim() || '';
    const boxTitle = block.querySelector('div:nth-child(7)')?.textContent.trim() || '';
    ...
    const contactContentHtml = () => {
        ...
        return `
            <div class="border-t w-full pt-10 border-solid border-t-black">
                ${sectionTitle ? `<h3 class="font-semibold text-5xl">${sectionTitle}</h3>` : ''}
    ...
    sectionContainer.innerHTML = `
        ${contactContentHtml()}
        <div class="relative">
            <div id="contact-map" class="bg-gray-100 h-[700px] w-[700px] flex items-center justify-center">
    ...
    if (!aemEnv) {
        ...
        block.append(sectionContainer);
        // Nascondi tutti gli elementi div originali in edit mode
        const allDivs = block.querySelectorAll('div:not(.container-layout-padding):not(.container-layout-padding *)');
        allDivs.forEach(div => {
            if (div.parentElement === block) {
