import { initDonationForm, loadAdyen, redirectExternalPayment } from '../../scripts/adyen-init.js';
...
export default async function decorate(block) {
  ...
  if (sessionStorage.length > 0) {
    ...
    containerSection.querySelector('#currency-amount-form')
      .classList
      .toggle('hidden', steps !== 1);
    containerSection.querySelector('#owner-information-form')
      .classList
      .toggle('hidden', steps !== 2);
    containerSection.querySelector('#adyen-form')
      .classList
      .toggle('hidden', steps !== 3);
  }
  ...
  submitOwnerInformationForm.addEventListener('submit', (e) => {
    ...
    const isFilled = JSON.parse(sessionStorage.getItem('formValue')).filled;
    ...
    containerSection.querySelector('#dropin-container')
      .classList
      .remove('hidden');
  });
  ...
  containerSection.querySelector('#max-amount-checkbox')
    .addEventListener('change', (e) => {
      const checked = e.target.checked;
      containerSection.querySelector('#button-form-owner-information')
        .classList
        .toggle('hidden', checked);
      containerSection.querySelector('#button-form-owner-information-link')
        .classList
        .toggle('hidden', !checked);
    });

  // ─── 5) Handle redirect response Box ────────────────────────────────────────────────────────────────
  const isRedirect = new URLSearchParams(window.location.search).get('redirectResult');
  if (isRedirect) {
    const sessionData = JSON.parse(sessionStorage.getItem('formValue'));
    console.log('redirectResult query param found, showing success box');

    const resultPayment = (message, type) => {
      const adyenForm = document.querySelector('#adyen-form');
      const currencyAmountForm = document.querySelector('#currency-amount-form');
      const ownerInformationForm = document.querySelector('#owner-information-form');
      const dropinResultContainer = document.querySelector('#dropin-container-result');
      adyenForm.classList.remove('hidden');
      currencyAmountForm.classList.add('hidden');
      currencyAmountForm.classList.remove('block');
      ownerInformationForm.classList.add('hidden');
      ownerInformationForm.classList.remove('block');
      dropinResultContainer.classList.add('block');
      dropinResultContainer.classList.remove('hidden');

      if (type === 'success') {
        console.log('✅ Payment authorized', message);
        dropinResultContainer.innerHTML = SuccessBox();
      }
      if (type === 'error' || type === 'failed') {
        console.log('Payment Failed', message);
        dropinResultContainer.innerHTML = ErrorBox();
      }
    ...
    await redirectExternalPayment(sessionData.country.code,
      (message) => resultPayment(message, 'success'),
      (message) => resultPayment(message, 'error'),
      (message) => resultPayment(message, 'failed'));
  }
            ...
            <div class="flex flex-col gap-2">
              ...
              <div class="flex w-full items-center gap-2">
                ...
                <label for="max-amount-checkbox" class="text-sm font-light cursor-pointer">
                  Receipt if higher amount then ${maxAmount}<span id="button-amount-span"></span>
                </label>
              </div>
            </div>
          </div>
  ...
  return `
    <div class="bg-white flex h-full w-full px-4 lg:p-8 flex-col gap-4">
      <div class="flex flex-col">
        ...
        <div class="hidden mt-3" id="dropin-container-result"></div>
...
const SuccessBox = () => {
  return `<div class="adyen-checkout__status adyen-checkout__status--success"><img height="88" class="adyen-checkout__status__icon adyen-checkout__image adyen-checkout__image--loaded" src="https://checkoutshopper-test.cdn.adyen.com/checkoutshopper/images/components/success.gif" alt="Successful Payment"><span class="adyen-checkout__status__text">Successful Payment</span></div>`;
};
const ErrorBox = () => {
  return `<div class="adyen-checkout__status adyen-checkout__status--error"><img class="adyen-checkout__status__icon adyen-checkout__image adyen-checkout__image--loaded" src="https://checkoutshopper-test.cdn.adyen.com/checkoutshopper/images/components/error.gif" alt="An error occurred" height="88"><span class="adyen-checkout__status__text">An error occurred</span></div>`;
};
