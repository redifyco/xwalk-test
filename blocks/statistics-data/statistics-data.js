...
export default function decorate(block) {
  ...
  `.trim();
    ...
    <div class="flex flex-col items-center lg:items-start">
      <h2 class="relative block px-4 text-center text-3xl font-semibold uppercase lg:px-16 lg:text-start lg:text-6xl 2xl:text-7xl ${backgroundImage ? 'text-white' : 'text-primary'}">
        ${title}
        <span class="absolute left-0 mt-5 hidden border-b-2 lg:block lg:w-[50%] xl:w-[90%] ${backgroundImage ? 'border-b-white' : 'border-b-primary'}"></span>
      </h2>
    </div>
    ...
    <div class="grid grid-cols-2 justify-items-center gap-8 px-4 lg:justify-items-start lg:gap-24 lg:px-24 xl:gap-32 xl:px-32">
      ${JSONStatisticsData.length > 0 ? JSONStatisticsData.map((item) => `
        <div class="flex flex-col items-center  lg:flex-row lg:items-end lg:gap-6 ${backgroundImage ? 'text-white' : 'text-primary'}">
